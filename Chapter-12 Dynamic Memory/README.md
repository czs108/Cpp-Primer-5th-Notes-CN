...
|                   操作                   |             含义              |
| :--------------------------------------: | :---------------------------: |
| `shared_ptr<T> sp`<br>`unique_ptr<T> up` |      默认初始化为空指针       |
...
|        操作         |                                          含义                                          |
| :-----------------: | :------------------------------------------------------------------------------------: |
| `weak_ptr<T> w(sp)` |                            `w`与`shared_ptr sp`指向相同对象                            |
|   `w.use_count()`   |                         返回与`w`共享对象的`shared_ptr`的数量·                         |
|    `w.expired()`    |                          若`w.use_count() == 0`，则返回`true`                          |
|     `w.lock()`      | 若`w.expired() == true`，则返回空`shared_ptr`；否则返回一个指向`w`的对象的`shared_ptr` |
