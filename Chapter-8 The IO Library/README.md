...
|   `s.clear()`    |   将流`s`的所有条件状态复位并将流置为有效    |
| `s.clear(flags)` |         将流`s`的条件状态置为`flags`         |
|  `s.rdstate()`   |               返回流的条件状态               |
...
|           操作           |                  含义                  |
| :----------------------: | :------------------------------------: |
|    `fstream fstrm(s)`    |  打开`s`文件，打开模式取决于`fstream`  |
| `fstream fstrm(s, mode)` |        以`mode`模式打开`s`文件         |
|     `fstrm.close()`      |                关闭文件                |
|    `fstrm.is_open()`     | 如果文件成功打开并尚未关闭，返回`true` |
...
|       操作        |            含义            |
| :---------------: | :------------------------: |
| `sstream strm(s)` | `strm`保存`string s`的拷贝 |
|   `strm.str()`    |   返回`strm`中的`string`   |
|   `strm.str(s)`   |  将`string s`拷贝至`strm`  |
