...
- new表达式调用名为`operator new`（或`operator new[]`）的标准库函数。该函数分配一块足够大、原始、未命名的内存空间以便存储特定类型的对象（或对象数组）。
...
`nothrow_t`类型是定义在头文件*new*中的一个结构体，这个类型不包含任何成员。头文件*new*还定义了一个名为`nothrow`的const对象，用户可以通过这个对象请求new的非抛出版本。
...
`malloc`函数接受一个表示待分配字节数的size_t参数，返回指向分配空间的指针，或者返回0以表示分配失败。`free`函数接受一个void\*参数，它是malloc返回的指针的副本，free将相关内存返回给系统。调用free(0)没有任何意义。
...
- 如果dynamic_cast语句的转换目标是引用类型，则抛出`bad_cast`异常（定义在头文件*typeinfo*中）。
...
typeid表达式的形式是`typeid(e)`，其中*e*可以是任意表达式或类型名称。typeid的结果是一个指向常量对象的引用，该对象的类型是标准库`type_info`（定义在头文件*typeinfo*中）或type_info的公有派生类型。
...
type_info类的精确定义会根据编译器的不同而略有差异。但是C++规定type_info必须定义在头文件*typeinfo*中，并且至少提供以下操作：
...
对于某种给定类型来说，`name`成员的返回值因编译器而异并且不一定与在程序中使用的名字一致。对于name返回值的唯一要求就是类型不同则返回的字符串必须有所区别。
...
使用标准库功能`mem_fn`（定义在头文件*functional*中）可以让编译器推断成员的类型。和function一样，mem_fn可以从成员指针生成可调用对象。但mem_fn可以根据成员指针的类型推断可调用对象的类型，无须显式指定。
...
当对象的值可能在程序的控制或检测之外被改变时（如子线程），应该将该对象声明为`volatile`。关键字volatile的作用是告知编译器不要优化这样的对象。
